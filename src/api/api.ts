...
export async function GET<T>(url: string): Promise<T> {
  const response = await fetch(url.startsWith("https") ? url : `${API_URL}${url}`);
...
async function POST(url: string, body: FormData | Record<string, unknown> = {}) {
...
export const login = (username: string, password: string) => POST(
  "/auth/login",
  loginAsFormData(username, password)
)
